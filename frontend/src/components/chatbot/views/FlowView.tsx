...
export function FlowView() {
  ...
  const load = async () => {
    setLoading(true)
    try {
      const [flow, edgeRes] = await Promise.all([
        api(API.flow),
        api(API.flowEdges).catch(() => ({ edges: [] })),
      ])
      setNodes(flow.nodes || [])
      setEdges(flow.edges?.length ? flow.edges : edgeRes.edges || [])
    } finally {
      setLoading(false)
    }
  }
  ...
  const canvasWidth = Math.max(1800, ...nodes.map((n) => n.positionX + NODE_WIDTH + 80), 800)
  const canvasHeight = Math.max(560, ...nodes.map((n) => n.positionY + NODE_HEIGHT + 80), 400)
