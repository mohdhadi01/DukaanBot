...
import { api, API, setDemoMode } from './api'
...
export function AppShell({ children, mode = 'app' }: { children: React.ReactNode; mode?: 'app' | 'demo' }) {
  // Must run during render (not useEffect) so child fetches use in-browser demo data
          ...
          <div className="flex items-center gap-2">
            <div className="hidden sm:flex items-center gap-2">
              {mode === 'app' && (
                ...
                >
                  {waStatus.status === 'connected' ? '● Live WhatsApp' : '○ Simulator only'}
                </Badge>
              )}
              <Badge
                variant="outline"
            ...
            <Button onClick={handleSeed} variant="outline" size="sm" className="gap-1.5">
              ...
              <span className="hidden sm:inline">{mode === 'demo' ? 'Reset Demo' : 'Reset Shop'}</span>
            </Button>
            {mode === 'app' ? (
              <Button variant="ghost" size="icon" className="h-8 w-8" onClick={async () => { await logout(); window.location.href = '/' }} aria-label="Sign out">
                <LogOut className="h-4 w-4" />
              </Button>
            ) : (
              <Button size="sm" className="bg-emerald-600 hover:bg-emerald-700 hidden sm:flex" asChild>
      ...
      <Dialog open={helpOpen} onOpenChange={setHelpOpen}>
        <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
          ...
          <div className="space-y-4 text-sm py-2">
            ...
            <HelpItem
              num="6"
              title="Connect WhatsApp (QR scan)"
              body="In Settings, click Connect WhatsApp and scan the QR with your shop phone — same as WhatsApp Web. Once connected, real customers can message you and the bot replies automatically."
            />
      ...
      <div className="max-w-md w-full text-center space-y-6 p-8 bg-white rounded-2xl shadow-xl border">
        ...
        <div className="space-y-2">
          ...
          <p className="text-muted-foreground text-sm">
            Complete onboarding to configure your WhatsApp bot and go live.
          </p>
        </div>
