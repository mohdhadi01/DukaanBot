...
function ChatPanel({
  ...
  const loadMessages = useCallback(async () => {
    const { messages } = await api(`${API.messages}?conversationId=${conversationId}`)
    const { shop } = await api(API.shop)
    setShop(shop)
    setMessages(messages)
    // Check if conversation ended
    const conv = await api(`${API.conversations}?conversationId=${conversationId}`)
    setEnded(conv.conversation?.status === 'resolved')
  }, [conversationId])
  ...
  const send = async (text?: string) => {
    ...
    try {
      ...
      // Replace temp with actual customer message + bot replies
      setMessages((m) => [...m.filter((x) => x.id !== tempId), ...res.messages])
      ...
      <div className="bg-white rounded-lg px-3 py-2 shadow-sm max-w-[80%] sm:max-w-md">
        <div className="flex items-center gap-1 mb-0.5">
          ...
          <span className="text-[10px] font-semibold text-slate-500">
            {isBot ? 'DukaanBot' : shop?.name || 'Shop'}
          </span>
        </div>
        <p className="text-sm whitespace-pre-wrap break-words">{message.text}</p>
