...
export async function loadFlowGraph(shopId: string): Promise<FlowGraph> {
  const [nodes, edges] = await Promise.all([
    FlowNode.find({ shopId }).lean(),
    FlowEdge.find({ shopId }).lean(),
  ])

  const parsedNodes: ParsedFlowNode[] = nodes.map((n: any) => ({
    id: String(n._id),
  ...
  const parsedEdges: ParsedFlowEdge[] = edges.map((e: any) => ({
    id: String(e._id),
    ...
    switch (currentNode.type) {
      ...
      case 'place_order': {
        ...
        const order = await Order.create({
          ...
          address: ctx.vars.address ? String(ctx.vars.address) : undefined,
          notes: ctx.vars.notes ? String(ctx.vars.notes) : undefined,
        ...
        ctx.vars.__lastOrderId = orderId
        ctx.cart = []
        const next = pickNext(currentNode, undefined)
        if (!next) {
          ctx.ended = true
...
async function buildMenuMessage(
  ...
  const query: Record<string, unknown> = { shopId, isAvailable: true }
  if (data.categoryIds && data.categoryIds.length > 0) {
    query.categoryId = { $in: data.categoryIds }
  }
  const products = await Product.find(query).sort({ name: 1 }).limit(30).lean()
...
export async function seedDefaultFlow(shopId: string) {
  const existing = await FlowNode.countDocuments({ shopId })
  ...
  const created = await FlowNode.insertMany(
    nodes.map((n) => ({
      shopId,
      type: n.type,
      title: n.title,
  ...
  const edges = [
    { src: String(startNode._id), tgt: String(welcomeNode._id) },
    { src: String(welcomeNode._id), tgt: String(questionNode._id) },
    { src: String(questionNode._id), tgt: String(menuNode._id), cond: 'menu', label: 'Menu/Order' },
    { src: String(questionNode._id), tgt: String(hoursNode._id), cond: 'hours', label: 'Hours' },
    { src: String(questionNode._id), tgt: String(humanNode._id), cond: 'human', label: 'Human' },
    { src: String(menuNode._id), tgt: String(addressNode._id) },
    { src: String(addressNode._id), tgt: String(notesNode._id) },
    { src: String(notesNode._id), tgt: String(placeOrderNode._id) },
    { src: String(placeOrderNode._id), tgt: String(endNode._id) },
    { src: String(hoursNode._id), tgt: String(questionNode._id) },
    { src: String(humanNode._id), tgt: String(endNode._id) },
  ]

  await FlowEdge.insertMany(
    edges.map((e) => ({
      shopId,
      sourceNodeId: e.src,
      targetNodeId: e.tgt,
